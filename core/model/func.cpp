...
static void bmm(memory m1, memory m2, memory m3) {
  auto Tensor = [](memory m) -> TensorFloat4D {
    ...
    Eigen::TensorMap<TensorFloat4D> tensor((float *)m.get_data_handle(), dim[0],
                                           dim[1], dim[2], dim[3]);
    return tensor;
  };
  ...
  for (int i = 0; i < d0; ++i) {
    ...
    for (int j = 0; j < d1; ++j) {
      t3_d0.chip<0>(j) = t1_d0.chip<0>(j).contract(t2_d0.chip<0>(j), dims);
    }
  }
}
...
void attention(const T *const query, const T *const key, const T *const &value,
  ...
  const dnnl::memory::dims q_sz = {p.mb, p.head_num, p.query_num, p.head_size};
